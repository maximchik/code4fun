...
function playNote(note, vol, period, type, df) {
    ...
    vol=(vol-osc.frequency.value/800>0.1)?(vol-osc.frequency.value/800):0.1;
 	...
 	for (k=strold.length; k<str.length; k++) {
	    ...
	    if (n>=0 && n<alfavit.length) {
			if (glas.indexOf(str.charAt(k))>=0) {
				j=n+3+shift;
				addNote(i,j+7,1,4,0,0);
				if (y==0) {
					addNote(i,j+2*7,1,8,0,0.2);
					addNote(i,j+5+1*7,1,8,0,-0.1);
				}   
				y++;
			} else {
				j=n+2+3+shift;
				addNote(i,j+2*7,0.5,4,1,0);
			}
			if (i%2==1) {
				j=n+3+shift;
				addNote(i,j+1*7,0.2,4,1,0.2);
				addNote(i,j+5+1*7,0.2,4,1,-0.1);        
			}
			i++;
	    } else {
